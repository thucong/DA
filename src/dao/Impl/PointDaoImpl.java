...
public class PointDaoImpl extends AbstractDAOImpl<PointModel> implements PointDao {
	...
	public List<PointModel> findAllByUserId(Long userId) {
		...
		sql.append("FROM diem INNER JOIN monhoc ON diem.id_monhoc = monhoc.id_monhoc WHERE id_user = ?");
		return query(sql.toString(), new PointMapper(), userId);
	}
	public List<PointModel> findAllBySubject(String subject){
		StringBuilder sql  = new StringBuilder("SELECT TenMH, id_user, id_monhoc, diembt, diemgk, diemck \");\r\n"
				+ "		sql.append(\"FROM diem INNER JOIN monhoc ON diem.id_monhoc = monhoc.id_monhoc WHERE tenMH like ? ");
		return query(sql.toString(), new PointMapper(), subject);
	}
